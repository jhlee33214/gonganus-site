...
export const CATEGORIES: Record<CategoryKey, { label: string; description: string }> = {
  cafe: { label: '카페', description: '카페 인테리어 촬영. 마감재의 질감과 자연광이 만드는 분위기를 사진과 영상으로 기록합니다.' },
  stay: { label: '숙박', description: '에어비앤비·독채·오피스텔 숙박 공간 촬영. 예약 전환을 높이는 객실 사진과 영상을 만듭니다.' },
  showroom: { label: '쇼룸', description: '가구·조명 브랜드 쇼룸 촬영. 제품과 공간이 함께 보이는 브랜드용 사진과 영상을 기록합니다.' },
  beauty: { label: '뷰티', description: '스파·뷰티숍 인테리어 촬영. 조명 조건을 정리해 마감재의 색을 정확하게 담습니다.' },
  residence: { label: '주거', description: '아파트·주택 인테리어 촬영. 시공 사례 포트폴리오를 위한 주거 공간 사진과 영상입니다.' },
  etc: { label: '기타', description: '키즈풀·프랜차이즈 매장·워크숍 등 다양한 상업 공간 촬영 사례입니다.' },
};
...
export function getCover(slug: string, cover: string): ImageMetadata {
  const found = getProjectImages(slug).find((i) => i.file === cover) ?? getProjectImages(slug)[0];
