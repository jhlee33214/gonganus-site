...
| `/work/` | 포트폴리오 전체 | 17건 그리드(최신순), 상단에 분류 링크 |
| `/work/category/{cafe,stay,showroom,beauty,residence,etc}/` | 분류별 목록 | 같은 그리드, 분류만 필터. 각 분류가 고유 제목·설명을 가진 별도 페이지 |
| `/work/{slug}/` | 프로젝트 상세 ×17 | 제목, 분류·연도, 1~2줄 설명, 사진 세로 나열(풀폭 1열, 세로 사진은 2열 짝지음), 이전·다음 프로젝트 링크 |
...
- 글꼴: Pretendard 한 종만(동적 서브셋 woff2, 400/500). 제목도 Pretendard. 세리프 없음. (사용자 지시 09-21)
...
- 원본 사진은 저장소에 포함(jpg 약 397장). 저장소 용량 1GB 한도 안에서 관리하기 위해 원본은 긴 변 2560px로 축소해 저장.
...
- 사진: 현 사이트 22페이지에서 `_rw_3840` 원본 URL 397장 수집·다운로드 → 프로젝트별 폴더 정리 → 2560px로 축소.
